...
const isBook = (item: Book | Video | ContentItem | NLBBook): item is Book =>
  (item as any).id !== undefined && (item as any).videoId === undefined && (item as any).filename === undefined && (item as any).isbn === undefined;

const isVideo = (item: Book | Video | ContentItem | NLBBook): item is Video =>
  (item as any).videoId !== undefined;

const isContentItem = (item: Book | Video | ContentItem | NLBBook): item is ContentItem =>
  (item as any).filename !== undefined;

const isNLBBook = (item: Book | Video | ContentItem | NLBBook): item is NLBBook =>
  (item as any).isbn !== undefined || ((item as any).records !== undefined && (item as any).coverUrl !== undefined);

const getItemId = (item: Book | Video | ContentItem | NLBBook) => {
  if (isBook(item)) return item.id;
  if (isVideo(item)) return item.videoId;
  if (isContentItem(item)) return item.id;
  if (isNLBBook(item)) return item.isbn || item.title;
  return '';
};
...
const getBookCover = (book: NLBBook): string => {
  ...
  return coverUrl || FALLBACK_THUMB;
};
...
export default function DiscoverPage() {
  // Core state
  const [mode, setMode] = useState<'books' | 'videos' | 'collection-books' | 'collection-videos' | 'nlb'>('books');
  ...
  // Modal state
  const [selectedItem, setSelectedItem] = useState<Book | Video | ContentItem | NLBBook | null>(null);
  ...
  const toggleFavourite = async (item: Book | Video | ContentItem | NLBBook, type: 'book' | 'video') => {
    if (!user) {
    ...
    try {
      if (exists) {
        ...
        if (isNLBBook(item)) {
          newFav.thumbnail = getBookCover(item);
          if (item.author) newFav.authors = [item.author];
        } else if ((isBook(item) || isContentItem(item)) && (item as any).authors) {
          newFav.authors = (item as any).authors;
        }
  ...
  const reportContent = async (item: Book | Video | ContentItem | NLBBook, type: 'book' | 'video') => {
    if (!user) {
  ...
  const handleModeChange = (newMode: 'books' | 'videos' | 'collection-books' | 'collection-videos' | 'nlb') => {
    ...
    else if (newMode === 'nlb') setNlbPage(1);
  };
  ...
  }, [mode, booksPage, videosPage, nlbPage, bucket, videoBucket]);
  ...
  const getSearchPlaceholder = () => {
    switch (mode) {
      ...
      case 'nlb': return "Search NLB catalogue e.g. harry potter, science…";
    ...
    <main className="bg-white">
      <div className="max-w-6xl mx-auto p-6 font-sans text-gray-900">
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          <button onClick={() => handleModeChange('nlb')} className={`px-4 py-2 rounded-lg ${mode === 'nlb' ? 'bg-black text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
            NLB Library
          </button>
        ...
        {!loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 mb-6">
            {getCurrentItems().map((item, index) => {
              // NLB Book Rendering
              if (mode === 'nlb' && isNLBBook(item)) {
                const book = item as NLBBook;
                const bookId = getItemId(book);
                const audienceTag = getAudienceDisplay(book.records?.[0]?.audience);
                
                return (
                  <div key={`${bookId}-${index}`} className="group relative border border-gray-200 rounded-xl overflow-hidden hover:shadow-lg transition-all bg-white cursor-pointer" onClick={() => handleContentItemClick(book)}>
                    {hasActivity(bookId, 'book') && (
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                          <div className="absolute bottom-2 left-2 right-2 flex gap-2">
                            <button onClick={(e) => { e.stopPropagation(); markAsRead(book); }} className="flex-1 bg-white/90 hover:bg-white text-gray-900 px-3 py-1.5 rounded-lg text-sm font-medium flex items-center justify-center gap-1">
                              ...
                              {hasActivity(bookId, 'book') ? 'Read' : 'Mark Read'}
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); toggleFavourite(book, 'book'); }} className={`bg-white/90 hover:bg-white px-3 py-1.5 rounded-lg ${isFavourite(bookId, 'book') ? 'text-red-500' : 'text-gray-900'}`}>
                              <Heart className={`w-3 h-3 ${isFavourite(bookId, 'book') ? 'fill-current' : ''}`} />
                            </button>
                          </div>
                        </div>
                      )}
                    ...
                    <div className="p-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1">
                        <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded">NLB</span>
                        {audienceTag && (
        ...
        {/* No Results */}
        {!loading && getCurrentItems().length === 0 && !collectionMessage && !nlbError && (
          <div className="text-center py-8 text-gray-600">
            {mode === 'books' && 'No books found.'}
            {mode === 'videos' && 'No videos found.'}
            {mode === 'nlb' && 'No NLB books found.'}
          ...
          <>
            ...
            {mode === 'nlb' && nlbBooks.length > 0 && (
              <div className="flex items-center justify-center gap-2 mt-6 flex-wrap">
                ...
                <button onClick={() => setNlbPage(nlbPage + 1)} disabled={!nlbHasMore} className="px-3 py-2 border rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
                  <ChevronRight className="w-4 h-4" />
                </button>
                {nlbTotalRecords > 0 && (
                  <button onClick={() => setNlbPage(Math.ceil(nlbTotalRecords / pageSize))} disabled={!nlbHasMore} className="px-3 py-2 border rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
                    <ChevronsRight className="w-4 h-4" />
                  </button>
                )}
              </div>
      ...
      {/* Selected Item Modal - Continuing in next message */}
{/* Selected Item Modal */}
      {selectedItem && (
        <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-50 text-gray-800 overflow-auto p-4">
          <div className="bg-white p-6 rounded-xl w-full max-w-[720px] relative max-h-[90vh] overflow-y-auto">
            ...
            {/* NLB Book Modal Content */}
            {isNLBBook(selectedItem) && (
              <>
                <div className="flex gap-4 mb-4">
                  <img 
                    src={getBookCover(selectedItem as NLBBook)} 
                  ...
                  <div className="min-w-0">
                    {(selectedItem as NLBBook).author && <p className="text-xs text-gray-600">{(selectedItem as NLBBook).author}</p>}
                    {(selectedItem as NLBBook).publishYear && <p className="text-[11px] text-gray-500 mt-1">Published: {(selectedItem as NLBBook).publishYear}</p>}
                    {(selectedItem as NLBBook).isbn && <p className="text-[11px] text-gray-500">ISBN: {(selectedItem as NLBBook).isbn}</p>}

                    {getAudienceDisplay((selectedItem as NLBBook).records?.[0]?.audience) && (
                      <div className="mt-2">
                ...
                {(selectedItem as NLBBook).subjects && (selectedItem as NLBBook).subjects!.length > 0 && (
                  <div className="mb-3">
                    <p className="text-xs font-medium text-gray-700 mb-1">Subjects:</p>
                    <div className="flex flex-wrap gap-1">
                      {(selectedItem as NLBBook).subjects!.map((subject, idx) => (
                    ...
                    {showPreview && (
                      <div className="mt-3 rounded-xl overflow-hidden border border-gray-200 shadow-sm bg-white">
                        <div className="p-4 max-h-96 overflow-y-auto">
                          {((selectedItem as NLBBook).summary || (selectedItem as NLBBook).records?.[0]?.summary?.[0]) && (
                            <div className="mb-4">
                          ...
                          {(selectedItem as NLBBook).records?.[0] && (
                            <div className="space-y-3">
                              <h4 className="text-sm font-semibold text-gray-900 mb-2">Book Information</h4>
                              
                              {(selectedItem as NLBBook).records![0].format && (
                                <div className="flex gap-2 text-sm">
                                  <span className="font-medium text-gray-700 min-w-[100px]">Format:</span>
                                  <span className="text-gray-600">{(selectedItem as NLBBook).records![0].format!.name}</span>
                                </div>
                              )}

                              {(selectedItem as NLBBook).records![0].publisher && (selectedItem as NLBBook).records![0].publisher!.length > 0 && (
                                <div className="flex gap-2 text-sm">
                                  <span className="font-medium text-gray-700 min-w-[100px]">Publisher:</span>
                                  <span className="text-gray-600">{(selectedItem as NLBBook).records![0].publisher!.join(', ')}</span>
                                </div>
                              )}

                              {(selectedItem as NLBBook).records![0].publishDate && (
                                <div className="flex gap-2 text-sm">
                                  <span className="font-medium text-gray-700 min-w-[100px]">Published:</span>
                                  <span className="text-gray-600">{(selectedItem as NLBBook).records![0].publishDate}</span>
                                </div>
                              )}

                              {(selectedItem as NLBBook).records![0].physicalDescription && (selectedItem as NLBBook).records![0].physicalDescription!.length > 0 && (
                                <div className="flex gap-2 text-sm">
                                  <span className="font-medium text-gray-700 min-w-[100px]">Description:</span>
                                  <span className="text-gray-600">{(selectedItem as NLBBook).records![0].physicalDescription!.join(', ')}</span>
                                </div>
                              )}

                              {(selectedItem as NLBBook).records![0].isbns && (selectedItem as NLBBook).records![0].isbns!.length > 0 && (
                                <div className="flex gap-2 text-sm">
                                  <span className="font-medium text-gray-700 min-w-[100px]">ISBN:</span>
                                  <span className="text-gray-600">{(selectedItem as NLBBook).records![0].isbns![0]}</span>
                                </div>
                              )}

                              <div className="flex gap-2 text-sm">
                                <span className="font-medium text-gray-700 min-w-[100px]">Availability:</span>
                                <span className={`${(selectedItem as NLBBook).records![0].availability ? 'text-green-600' : 'text-orange-600'}`}>
                                  {(selectedItem as NLBBook).records![0].availability ? 'Available' : 'Currently unavailable'}
                                </span>
                              </div>
                            </div>
                          )}
                        ...
                        <div className="p-3 bg-gray-50 border-t">
                          ...
                          <a 
                            href={getNLBCatalogueUrl(selectedItem as NLBBook)}
                            target="_blank" 
            ...
            <div className="flex flex-col gap-2 mb-3">
              {user && (
                <>
                  ...
                  {isNLBBook(selectedItem) && (
                    <button onClick={() => hasActivity(getItemId(selectedItem), 'book') ? removeActivity(getItemId(selectedItem), 'book') : markAsRead(selectedItem as any)} className={`px-3 py-2 rounded-lg border flex items-center gap-2 ${hasActivity(getItemId(selectedItem), 'book') ? 'bg-green-100 text-green-700 border-green-300' : 'border-gray-300 hover:bg-green-50'} ${!canAccessContent('book') ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!canAccessContent('book')}>
                      <BookOpen className="w-4 h-4" />
                      {hasActivity(getItemId(selectedItem), 'book') ? 'Mark as Unread' : 'Mark as Read'}
                    </button>
                  )}
                  ...
                  <button onClick={() => toggleFavourite(selectedItem, (mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video')} className={`px-3 py-1 rounded-lg border ${!canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video') ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video')}>
                    {isFavourite(getItemId(selectedItem), (mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video') ? '★ Remove Favourite' : '☆ Add Favourite'}
                  </button>
                  <button onClick={handleLeaveReview} className={`px-3 py-1 rounded-lg border text-green-600 ${!canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video') ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video')}>
                    Leave Comment
                  </button>
                  <button onClick={() => reportContent(selectedItem, (mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video')} className="px-3 py-1 rounded-lg border text-red-600">
                    Report Content
                  </button>
                </>
              )}
            ...
            <div className="mt-3">
                  ...
                  <div key={r.id} className="border border-[#eee] p-3 rounded-lg mb-2 text-sm">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex items-center gap-2">
                        {user && user.uid !== r.userId && canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video') && (
                        ...
                        {(!user || user.uid === r.userId || !canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video')) && getHeartCount(r.id) > 0 && (
              ...
              {user && canAccessContent((mode === 'books' || mode === 'collection-books' || mode === 'nlb') ? 'book' : 'video') ? (
                <div className="mt-2">
